...
-- Number of movies are released each year
-- Count id by making a group by year
SELECT 
...
-- highest number of movies is produced in the March
-- lowest number of movies is produced in the December
...
-- Movie can be counted if multiple countries satisfy the condition any one by like operator
SELECT 
...
-- Only count if country is only India or USA
SELECT 
    country, COUNT(id) AS no_of_movie
FROM
    movie
WHERE
...
SELECT 
    genre, year, COUNT(movie_id) AS number_of_movies
FROM
    genre AS g
        INNER JOIN
...
WHERE
    genre = 'thriller';
-- Rank of the ‘thriller’ genre of movies among all the genres in terms of number of movies produced is 3
...
ORDER BY movie_count DESC;
 ...
 -- Dream Warrior Pictures or National Theatre Live both are have same rank
...
FROM (
    SELECT 
        ...
        RANK() OVER(ORDER BY SUM(avg_rating*r.total_votes)/SUM(r.total_votes) DESC) AS actor_rank
    FROM 
        movie AS m 
    INNER JOIN 
        ratings AS r ON m.id = r.movie_id 
...
FROM (
    SELECT 
        ...
        RANK() OVER(ORDER BY SUM(avg_rating*r.total_votes)/SUM(r.total_votes) DESC) AS actress_rank
    FROM 
        movie AS m 
    INNER JOIN 
        ratings AS r ON m.id = r.movie_id 
...
-- Top five actresses in Hindi movies released in India based on their average ratings are Taapsee Pannu, Kriti Sanon, Divya Dutta, Sonakshi Sinha, Kriti Kharbanda
...
SELECT 
    ...
    AVG(ROUND(AVG(duration), 2)) OVER(PARTITION BY genre ORDER BY genre ROWS 10 PRECEDING) AS moving_avg_duration
FROM 
    movie AS m 
INNER JOIN 
    genre AS g ON m.id = g.movie_id
